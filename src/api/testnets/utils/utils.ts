...
import {
  ...
  SaleKind,
  Transaction,
} from '../../../utils/types/opensea/types';

const NULL_ADDRESS = constants.NULL_ADDRESS;
const INVERSE_BASIS_POINT = constants.INVERSE_BASIS_POINT;
...
export function estimateCurrentPrice(order: Order, secondsToBacktrack = 30, shouldRoundUp = true) {
  ...
  return shouldRoundUp ? exactPrice.integerValue(BigNumber.ROUND_CEIL) : exactPrice;
}
...
export const assetFromJSON = (asset: any): OpenSeaAsset => {
  ...
  const fromJSON: OpenSeaAsset = {
    ...
    owner: accountFromJSON(asset.owner),
    creator: accountFromJSON(asset.creator),
    topBid: asset.topBid,
